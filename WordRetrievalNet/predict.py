...
def get_boxes(score, geo, cls_score_thresh=0.9, bbox_nms_overlap=0.4):
    ...
    if xy_text.size == 0:
        return (
            np.zeros(shape=(0,9), dtype=np.float32),
            np.bool_([])
        )
    ...
    boxes, keep = nms_locality(boxes, bbox_nms_overlap)
    return boxes, keep
...
class Predictor:
    def __init__(self, config, gpu_id=None):
        self.config = config
        self.config['tester']['output_dir'] = self.config['tester']['output_dir']#os.path.join(str(pathlib.Path(os.path.abspath(__name__)).parent), self.config['tester']['output_dir'])
        print(f"{self.config['tester']['output_dir']=}")
    ...
    def predict(self):
        ...
        for i, (img_file, gt_file) in enumerate(zip(self.test_img_files, self.test_gt_files)):
            ...
            for_cal_map[img_name] = {"pred_coord": predict_boxes, "pred_embedding": pred_embedding_list,
                                     ...
                                     "gt_embedding": [self.words_embeddings[test_word] for test_word in gt_words_list]
                                     }
    ...
    def cal_f1(self):
        ...
        pred_result = np.load(self.npy_path, allow_pickle=True).tolist()
        for_cal_map = pred_result["for_cal_map"]
    ...
    def cal_map(self):
        ...
        for over_thresh in self.overlap_thresh:
            mAP_qbs, mR_qbs = cal_map(qbs_queries, qbs_targets, db, db_targets, gt_targets, joint_boxes,
                                      all_overlaps, self.query_nms_overlap, over_thresh, qbs_words, num_workers=3)
...
if __name__ == '__main__':
    ...
    print("Finished with the exportation of the predictions")
    predictor.cal_f1()   # calculate Recall Precision F1
    predictor.cal_map()  # calculate MAP
    for query in args.queries:
        predictor.query_word_string(query, args.topN)  # query word string defined by users
