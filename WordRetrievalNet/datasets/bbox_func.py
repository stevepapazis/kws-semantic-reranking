...
def rotate_vertices(vertices, theta, anchor=None):
    ...
    """
    # v = vertices.reshape(-1,4,2).swapaxes(1,2)
    # if anchor is None: anchor = v[...,:1]
    # rotate_mat = get_rotate_mat(theta)
    # res = rotate_mat @ (v - anchor)
    # res = (res + anchor).swapaxes(1,2).reshape(-1,8)
    # return res.squeeze()
...
def find_min_rect_angle(vertices):
    ...
    angle_list = np.arange(-90, 90, step=angle_interval, dtype=np.float64)/180*np.pi
    # rotated = rotate_vertices(vertices,  angle_list)
    area_list = []
    for theta in angle_list:
        rotated = rotate_vertices(vertices, theta)#theta / 180 * np.pi)
...
def crop_img(img, bbs, length):
    ...
    bbs = BoundingBoxes(
        bbs.numpy() - [minx, miny, minx, miny],
        format="xyxy",
        canvas_size=img.shape[-2:]
    )
    
    h, w = img.shape[-2:]
    # assert (h >= length and w >= length)
    
    crop = transforms.RandomCrop(length, pad_if_needed=True)
    return crop(img, bbs)
...
def random_scale(img, bbs, long_size=2048, ratio=0.2):
    """ random scale image to aug data """
    h, w = img.shape[-2:]
    ratio = 1 + np.random.uniform(-ratio, ratio)
    scale = ratio * long_size / max(w,h)
    resize = transforms.Resize( (int(h*scale), int(w*scale)) )
    
    return resize(img, bbs)
...
def rotate_img(img, bbs, angle_range=10):
    ...
    rotation = transforms.RandomRotation(angle_range, interpolation=transforms.InterpolationMode.BILINEAR)
    return rotation(img, bbs)
