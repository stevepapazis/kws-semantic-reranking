...
# from utils.gw_dataset_almazan import GWDataset
...
parser.add_argument('--result_output_path', action='store', type=str)
...
train_set = dataset_constructor(dataset_folder, subset='train', segmentation_level='fword', fixed_size=(64 + 1 * 32, 256 + 0 * 128), transforms=aug_transforms)  # (128, 1024))
test_set = dataset_constructor(dataset_folder, subset='test', segmentation_level='fword', fixed_size=(64 + 1 * 32, 256 + 0 * 128))


#!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#form_val_set = IAMDataset(dataset_folder, subset='val', segmentation_level='form', fixed_size=None)
form_val_set = dataset_constructor(dataset_folder, subset='test', segmentation_level='form', fixed_size=None)
#form_val_set = dataset_constructor(dataset_folder, subset='val', segmentation_level='form', fixed_size=None, transforms=None)


ndisplay = 50
# use only Ns forms for validation - just to see if the process is going in the right direction
Ns = 5#10 # iam seg-free #docs eval

# augmentation using data sampler
train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=3, drop_last=False)#True)
...
if args.model_load_path is None:
    ...
    cnn = torch.load(args.model_load_path)
    # try:
    #     cnn.load_state_dict(torch.load(args.model_load_path).state_dict())
    # except:
    #     print(f"Error loading model at {args.model_load_path}")
    #     raise
...
def train(epoch):
    ...
    for iter_idx, (img, transcr, bbox) in enumerate(train_loader):
        ...
        if iter_idx % ndisplay == (ndisplay-1):
            ...
            logger.info(f'%f %f', loss1.item(), loss2.item())
            ...
            with torch.no_grad():
                ...
                ycnt, _, yctc = cnn(Variable(tst_img.cuda(gpu_id)).unsqueeze(0), bbox.cuda(gpu_id).unsqueeze(0))
...
class EvalArgs:
    ...
    iou_mode: int = 0
    doc_scale: float = 1.0
    masked_form: bool = (dataset == 'iam') ##or (dataset == 'gw') ### TODO: what about this???
...
cnn.eval()
best_map = seg_free_eval(form_val_set, cnn, classes, eval_args, Ns=Ns)
for epoch in range(1, max_epochs + 1):
    ...
    print(f"{tmp_map=} > {best_map=}? {tmp_map > best_map}")
    if tmp_map > best_map:
        print('Saving net !!')
