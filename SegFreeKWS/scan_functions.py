...
def scan_page(scale_cmap, steps, valid_map, starget, prob_thres=.05):
    ...
    bboxes = [np.int_(x) for x in range(0)]
    carea_ratio = [np.float32(x) for x in range(0)]
...
def cos_scores(cmap, bboxes, cnt_target, levels=1, phoc_activate=False):
    ...
    #valid_inds = [np.int_(x) for x in range(0)]
...
def ctc_scores(rmap, bboxes, labels, ctc_thres, ctc_mode=2):
    ...
    nbboxes, nscores = [np.int_(x) for x in range(0)], [np.float32(x) for x in range(0)]
    for bbox in bboxes:
        sj, si, ej, ei = bbox
        sj = max(sj, 0)
        si = max(si, 0)
