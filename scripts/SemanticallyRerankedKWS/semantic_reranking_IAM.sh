...
python "../../SemanticallyRerankedKWS/semantically_reranked_kws.py" --ground-truth-location $GROUND_TRUTH --wrn-result-location $WRN_OUTPUT --segfreekws-result-location $SEGFREEKWS_OUTPUT --save-results-location $KWS_RESULTS --avoid-cache --dataset $DATASET --skip-mAP-computation ##--skip-WRN-mAP-computation ##--skip-SegFreeKWS-mAP-computation


SEGFREEKWS_MODULE="../../SegFreeKWS/"   

### Compute the semantic similarity 
SEGFREEKWS_MODEL="../../pretrained_models/segfreekws_iam_best.pt"
python "../../SemanticallyRerankedKWS/produce_semantic_embeddings.py" --path-to-segfreekws-module $SEGFREEKWS_MODULE --path-to-kws-results $KWS_RESULTS --path-to-gw $DATASET_PATH --test-fold $FOLD --path-to-segfreekws-decoder $SEGFREEKWS_MODEL --avoid-cache
